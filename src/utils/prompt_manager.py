...
class PromptManager:
    ...
    def get_prompt(self, prompt_type: str, enabled_mcp_servers: Optional[list] = None, **kwargs: Any) -> Optional[str]:
        ...
        # ソースAgent情報を動的に統合
        if "source_agents" in self.prompts:
            source_agents_config = self.prompts["source_agents"]

            # 全てのソースAgentの情報を整形して統合
            agents_info_list = []
            for agent_key, agent_config in source_agents_config.items():
                if agent_key.startswith("#"):  # コメント行をスキップ
                    continue

                agent_info = f"""**{agent_config.get('title', agent_key)}**:
...
"""
                # ラベルの追加
                for label in agent_config.get("labels", []):
                    label_type = {"report": "日次レポート", "weekly-report": "週次レポート", "monthly-report": "月次レポート"}.get(label, label)
                    agent_info += f"- `label:{label}` - {label_type}\n"

                # 期間フィルタリング方法の追加（プロンプトから直接取得）
                period_filtering = agent_config.get("period_filtering", "")
                if period_filtering:
                    agent_info += f"\n{period_filtering}\n"
            ...
            # 全てのAgent情報を統合
            kwargs["source_agents"] = "\n---\n\n".join(agents_info_list)

        # キーワードを動的に統合（後方互換性のため残す）
        ...
        if "template" in prompt_config:
            ...
            # ソースAgent情報が含まれている場合は置換
            if "{source_agents}" in prompt_text and "source_agents" in kwargs:
                prompt_text = prompt_text.replace("{source_agents}", kwargs["source_agents"])
            # キーワードが含まれている場合は置換（後方互換性）
            if "{key_words}" in prompt_text and "key_words" in kwargs:
                prompt_text = prompt_text.replace("{key_words}", kwargs["key_words"])
            # key_urlsが含まれている場合は置換（後方互換性）
