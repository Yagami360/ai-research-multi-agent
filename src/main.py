"""
AI Research Multi Agent メインアプリケーション
"""
...
from .agent import AIResearchMultiAgent
...
def main() -> None:
    """メイン関数"""
    logger.info("Started AI Research Multi Agent")
    logger.debug(f"settings: {settings}")

    parser = argparse.ArgumentParser(
        description="AI Research Multi Agent - AI技術動向と論文の統合分析レポート生成ツール\n"
        "2つのソースAgent（ai-tech-catchup-agent, ai-paper-catchup-agent）のレポートを統合分析し、\n"
        "スキル成長とビジネス創出のための戦略的アクションプランを提案します。",
    ...
    parser.add_argument(
        ...
        help="レポートモード (weekly: 週次, monthly: 月次, test: テスト。指定なし: 統合分析レポート)",
    )
    parser.add_argument(
        "--model",
        type=str,
